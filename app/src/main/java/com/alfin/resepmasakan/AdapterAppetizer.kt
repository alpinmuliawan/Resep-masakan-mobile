...
class AdapterAppetizer(
    private val context: Context,
    private val resepList: MutableList<Recipe> // MutableList untuk manipulasi daftar
) : RecyclerView.Adapter<AdapterAppetizer.ResepViewHolder>() {

    // Referensi ke Firebase Database
    private val database = FirebaseDatabase.getInstance("https://resepmasakan-2-default-rtdb.asia-southeast1.firebasedatabase.app/").getReference("Appetizer")
    ...
    class ResepViewHolder(view: View) : RecyclerView.ViewHolder(view) {
        ...
        val tombol: Button = view.findViewById(R.id.tombol)
    }

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ResepViewHolder {
        val view = LayoutInflater.from(context).inflate(R.layout.list_resep, parent, false)
