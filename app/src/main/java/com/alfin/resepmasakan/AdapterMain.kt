...
) : RecyclerView.Adapter<AdapterMain.ResepViewHolder>() {

    // Referensi ke Firebase Database
    private val database = FirebaseDatabase.getInstance("https://resepmasakan-2-default-rtdb.asia-southeast1.firebasedatabase.app/").getReference("Maincourse")

    // ViewHolder untuk item
    class ResepViewHolder(view: View) : RecyclerView.ViewHolder(view) {
        val judul: TextView = view.findViewById(R.id.judul2)
        val tombol: Button = view.findViewById(R.id.tombol2)
    }

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ResepViewHolder {
        val view = LayoutInflater.from(context).inflate(R.layout.activity_list_resepmaincourse, parent, false)
